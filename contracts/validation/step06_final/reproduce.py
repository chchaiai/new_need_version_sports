...
from generator_support import nullable_scalar_mappings, check_template
...
def main():
    ...
    src=repo/'contracts/validation/step05_teaching';final=repo/'contracts/validation/step06_final';prior=repo/'contracts/validation/step03_workflow';old=repo/'contracts/validation/step02_discriminators';spec=repo/'contracts/openapi.yaml'
    ...
    run('03b-final-structural',py+[final/'check_final.py','--spec',spec,'--output',out/'final-structural.json'])
    import yaml
    is_rc=yaml.safe_load(spec.read_bytes())['info']['x-contract-status']=='RC'
    ready=run('04-readiness',py+[repo/'contracts/scripts/check_rc_readiness.py'],0 if is_rc else 1)
    assert ('PASS' if is_rc else 'DRAFT') in ready.stdout+ready.stderr
    run('05-workflow-cases',py+[src/'run_checks.py','--spec',spec,'--output',out/'checks'])
    fourth=repo/'contracts/validation/step04_courses'
    run('05a-step04-regression',py+[fourth/'run_checks.py','--spec',spec,'--output',out/'prior-courses'])
    run('05a2-step04-js',[node,fourth/'runtime_web.cjs',out/'prior-courses/runtime-input.json',tool['ajvRoot'],out/'prior-courses/web-result.json'])
    ...
    combined=[]
    for folder in ['old-mapping','prior-workflow','prior-courses','checks']:
        data=json.loads((out/folder/'runtime-input.json').read_text(encoding='utf-8'));assert data['candidateSha256']==sha(spec)
        for row in data['cases']:combined.append({**row,'name':folder+'/'+row['name']})
    ...
    good=[r for r in combined if r['expectedValid']]
    ...
    closed=[('RosterDraftRow','state'),('ResolvedEnduranceValues','timeRepresentation'),('ManualModeWindow','state'),
        ('CreateStudentApplicationRequest','applicationType'),('SwitchSystemModeRequest','targetMode'),
        ('SubmitExerciseRecordRequest','submissionRoute'),('AppendRecordReviewRequest','action'),('CorrectExerciseRecordReviewRequest','result')]
    for i,(name,field) in enumerate(closed):
        ts.append(f'type Closed{i} = S["{name}"]["{field}"];')
        for j,value in enumerate(['UNKNOWN',None,1]):
            ts += ['// @ts-expect-error invalid discriminator must fail in the actual generated type',f'const bad{i}_{j}: Closed{i} = '+json.dumps(value)+';']
    ...
    mappings,imports=nullable_scalar_mappings(yaml.safe_load(spec.read_bytes()))
    config['schemaMappings'].update(mappings);config['importMappings'].update(imports)
    check_template(final,tool['generatorJar'])
    ...
    wrappers += ['SubmitExerciseRecordRequest','SubmitOrdinaryExerciseRecordRequest','SubmitSwimmingExerciseRecordRequest','SubmitOfflineSwimmingExerciseRecordRequest',
        'AppendRecordReviewRequest','PassExerciseRecordRequest','ReturnExerciseRecordRequest','InvalidateExerciseRecordRequest',
        'CorrectExerciseRecordReviewRequest','CorrectExerciseRecordValidRequest','CorrectExerciseRecordInvalidRequest','MaterialManifestItem']
    for label in ['base-models','selected-wrappers']:
        config['outputDir']=str(out/label)
        if label=='selected-wrappers':
            config['library']='jvm-retrofit2';config['additionalProperties']['generateOneOfAnyOfWrappers']='true';config['templateDir']=str(final/'templates')
    ...
    # All four existing suites must refer to the same final candidate bytes.
    factories=['CreateStudentApplicationRequest','ReviseEnduranceRuleTableRequestChange','ReviseEnduranceRuleTableRequest','SwitchSystemModeRequest',
        'SubmitExerciseRecordRequest','AppendRecordReviewRequest','CorrectExerciseRecordReviewRequest']
    runtime_input=out/'all-runtime-input.json';runtime_input.write_bytes((json.dumps({'candidateSha256':sha(spec),'spec':data['spec'],'cases':combined,'wrapperFactories':factories},ensure_ascii=False)+'\n').encode())
    deps_manifest=json.loads((Path(tool['finalRuntimeRoot'])/'download-manifest.json').read_text(encoding='utf-8'))
    for f in deps_manifest:assert sha(f['path'])==f['sha256']
    runtime_cp+=os.pathsep+os.pathsep.join(f['path'] for f in deps_manifest if f['path'].endswith('.jar'))
    run('16a-strict-jvm-probe-compile',[Path(tool['javaHome'])/'bin/javac.exe','-encoding','UTF-8','--release','17','-cp',runtime_cp,'-d',classes,final/'jvm/ContractRuntimeProbe.java'])
    run('16b-strict-jvm-runtime',[java,'-cp',runtime_cp+os.pathsep+str(classes),'ContractRuntimeProbe',runtime_input,out/'strict-jvm-result.json'])
    ...
    result={'candidateSha256':sha(spec),'generatedFiles':identity,'commands':logs,'allExpectedExits':True,
            'kotlinModelsCompiled':len(model_manifest),'selectedWrapperModels':len(wrappers),'allStepTypeScriptLegalFixtures':len(good),
            'allStepTypeScriptNegativeAssertions':len(closed)*3,'priorStep03KotlinUnionRuntime':'PASS_ISOLATED_JVM_SCHEMA_AND_GENERATED_BINDING_PHASE6_APP_INTEGRATION_NOT_RUN',
            'rcReadiness':'PASS' if is_rc else 'EXPECTED_BLOCKED_DRAFT','backend':'NOT_RUN_PHASE7_0_COMPATIBILITY_AND_PHASE7_RUNTIME',
            'newModelAndroidRuntime':'PASS_ISOLATED_JVM_SCHEMA_AND_GENERATED_BINDING_PHASE6_APP_INTEGRATION_NOT_RUN','priorWorkflowReport':'prior-workflow/python-result.json','schemaReport':'checks/python-result.json','javascriptReport':'checks/web-result.json'}
