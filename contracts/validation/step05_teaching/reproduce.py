...
def main():
    ...
    src=repo/'contracts/validation/step05_teaching';prior=repo/'contracts/validation/step03_workflow';old=repo/'contracts/validation/step02_discriminators';spec=repo/'contracts/openapi.yaml'
    ...
    for i,(name,field) in enumerate([('RosterDraftRow','state'),('ResolvedEnduranceValues','timeRepresentation'),('ManualModeWindow','state')]):
    ...
    result={'candidateSha256':sha(spec),'generatedFiles':identity,'commands':logs,'allExpectedExits':True,
            'kotlinModelsCompiled':len(model_manifest),'step02SelectedWrapperModels':len(wrappers),'step05TypeScriptLegalFixtures':len(good),
            'step05TypeScriptNegativeAssertions':9,'priorStep03KotlinUnionRuntime':'PENDING_STEP6_CANDIDATE_GATE_AND_PHASE6_CONSUMER_VALIDATION',
