...
def main():
    ...
    src=repo/'contracts/validation/step03_workflow';old=repo/'contracts/validation/step02_discriminators';spec=repo/'contracts/openapi.yaml'
    ...
    for i,(name,field) in enumerate([('SubmitExerciseRecordRequest','submissionRoute'),('AppendRecordReviewRequest','action'),('CorrectExerciseRecordReviewRequest','result')]):
    ...
    result={'candidateSha256':sha(spec),'generatedFiles':identity,'commands':logs,'allExpectedExits':True,
            'kotlinModelsCompiled':len(model_manifest),'step02SelectedWrapperModels':len(wrappers),'step03TypeScriptLegalFixtures':len(good),
            'step03TypeScriptNegativeAssertions':9,'step03KotlinUnionRuntime':'PENDING_STEP6_CANDIDATE_GATE_AND_PHASE6_CONSUMER_VALIDATION',
            'rcReadiness':'EXPECTED_BLOCKED_DRAFT','backend':'NOT_RUN_PHASE7_0_COMPATIBILITY_AND_PHASE7_RUNTIME',
            'schemaReport':'checks/python-result.json','javascriptReport':'checks/web-result.json'}
