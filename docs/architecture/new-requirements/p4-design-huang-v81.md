...
> 本轮H候选设计版本：`P4H-FINAL-ALIGN-1.4`。1.0完成F、G1、G3工程DoD，1.1对齐P-01～04与Contract起始输入，1.2关闭GAP-H13，1.3与Z/G2/矩阵/联合汇总建立最终替代关系及Phase 2映射。本文较早章节中的PENDING、canonical UNKNOWN、Phase 4 IN_PROGRESS或Phase 5 LOCKED均为带版本历史快照，当前结论只认§19.2～19.8。
...
H侧消费Z拥有的正式受理事实：普通运动首次正式受理必须满足`acceptedAt < endedAt + 24h`；受理时锁定同一材料批次，全部必需对象的权威传输完成必须满足`completedAt < acceptedAt + 30m`。等于端点即超时，使用服务器完整精度与UTC时刻裁决，客户端倒计时不具权威性。课程关闭、成员移出或收尾不能截断边界前已由服务器确认的合法Session/Record：尚未首次受理材料时保留原首次受理窗口，已受理时保留原同批传输窗口；不能要求学生必须在生命周期变化前完成首次受理。该入口仍核验登录、本人身份与原资源归属，不新建运动、不恢复完整成员权限、不迁移到新课程。该规则不覆盖游泳15分钟首次受理、游泳离线异常队列或教师退回补证24/72小时专门规则。
