...
本目录实现 Phase 7 v3 + H 审核 SEC-01 修正的 `P7-Z-0 / 7.0 Foundation`。当前范围是工程与验证基础、内部健康检查，以及 `CertificationKind` 四层表示/持久化探针。业务 API、认证、课程和审核尚未实现；7.0 通过 H 的定向复核、最终 Reviewer 接受及 GitHub 合并核验后才进入波次 A。
